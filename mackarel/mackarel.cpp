...
//    p(0x76); // HALT (for debugging)
    ...
    if (gOptExecAddress)
    {
        p(0x21); p((gOptExecAddress) & 0xff); p((gOptExecAddress >> 8) & 0xff);  // ld hl, #optexecaddress
    }
...
void parse_commandline(int parc, char ** pars)
{
    ...
    for (i = 0; i < parc; i++)
    {
        if (pars[i][0] == '-')
        {
            ...
            if (stricmp(pars[i], "-execaddr") == 0)
            {
                i++;
                gOptExecAddress = strtol(pars[i],0,0);
            }
            else
...
void sanity_checks()
{
    ...
    if (gOptHiBlock && gOptHiBlockAddr < gExecAddr + gImageSize) 
